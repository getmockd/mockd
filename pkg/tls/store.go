...
// CertExpiryGracePeriod is the duration before expiry at which a certificate
// is considered "expiring soon" and will be auto-regenerated by EnsureCertificate.
const CertExpiryGracePeriod = 24 * time.Hour

// IsCertExpired reports whether the certificate has expired or will expire
// within the given grace period. A zero grace period checks only actual expiry.
func IsCertExpired(cert *x509.Certificate, grace time.Duration) bool {
	return time.Now().Add(grace).After(cert.NotAfter)
}

// CheckCertExpiry returns a descriptive error if the certificate is expired
// or will expire within the grace period. Returns nil if the certificate is valid.
// This is intended for user-provided certificates where auto-regeneration is
// not appropriate — the caller should surface the error to the user.
func CheckCertExpiry(cert *x509.Certificate, grace time.Duration) error {
	now := time.Now()
	if now.After(cert.NotAfter) {
		return fmt.Errorf("certificate expired on %s (%s ago)",
			cert.NotAfter.Format("2006-01-02 15:04:05"),
			now.Sub(cert.NotAfter).Truncate(time.Minute))
	}
	if now.Add(grace).After(cert.NotAfter) {
		return fmt.Errorf("certificate expires on %s (within %s grace period)",
			cert.NotAfter.Format("2006-01-02 15:04:05"),
			grace)
	}
	return nil
...
// EnsureCertificate ensures a valid certificate exists at the given paths.
// If files don't exist, generates new ones. If they exist but the certificate
// is expired or will expire within CertExpiryGracePeriod (24h), regenerates
// and overwrites the files. If they exist and are valid, loads them.
func EnsureCertificate(cfg *CertificateConfig, certPath, keyPath string) (*GeneratedCertificate, error) {
	...
	if certErr == nil && keyErr == nil {
		// Both files exist, load and check expiry
		loaded, err := LoadCertFromFiles(certPath, keyPath)
		if err != nil {
			// Files exist but are corrupt/unreadable — regenerate
			return GenerateAndSave(cfg, certPath, keyPath)
		}
		if !IsCertExpired(loaded.Certificate, CertExpiryGracePeriod) {
			return loaded, nil
		}
		// Expired or expiring soon — regenerate
		return GenerateAndSave(cfg, certPath, keyPath)
	}
