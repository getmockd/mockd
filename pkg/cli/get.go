...
Run 'mockd get --help' for more options`)
	...
	// Response headers
	if mock.Response != nil && len(mock.Response.Headers) > 0 {
		fmt.Println("  Headers:")
	...
	// Match headers
	if mock.Matcher != nil && len(mock.Matcher.Headers) > 0 {
		fmt.Println("  Match Headers:")
		for key, value := range mock.Matcher.Headers {
			fmt.Printf("    %s: %s\n", key, value)
		}
	}
	...
	if mock.Matcher != nil && len(mock.Matcher.QueryParams) > 0 {
		...
		for key, value := range mock.Matcher.QueryParams {
			fmt.Printf("    %s: %s\n", key, value)
		}
	}

	// Response body
	if mock.Response != nil && mock.Response.Body != "" {
		fmt.Println("  Body:")
		// Indent body content
	...
	// Delay
	if mock.Response != nil && mock.Response.DelayMs > 0 {
		fmt.Printf("  Delay:    %dms\n", mock.Response.DelayMs)
	}
