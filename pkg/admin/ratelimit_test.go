...
func TestNewPerIPLimiter(t *testing.T) {
	t.Run("creates rate limiter with default values", func(t *testing.T) {
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{
			Rate:  DefaultRateLimit,
			Burst: DefaultBurstSize,
		})
		defer rl.Stop()

		// Verify it works by allowing a request
...
func TestPerIPLimiter_Allow(t *testing.T) {
	t.Run("allows requests within limit", func(t *testing.T) {
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{Rate: 10, Burst: 5})
		defer rl.Stop()
	...
	t.Run("blocks requests when bucket is empty", func(t *testing.T) {
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{Rate: 10, Burst: 2})
		defer rl.Stop()
	...
	t.Run("refills tokens over time", func(t *testing.T) {
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{Rate: 100, Burst: 2})
		defer rl.Stop()
...
func TestPerIPLimiter_Middleware(t *testing.T) {
	...
	t.Run("sets rate limit headers on allowed requests", func(t *testing.T) {
		...
		middleware := ratelimit.Middleware(rl, ratelimit.WithTextResponse())(handler)
	...
	t.Run("returns 429 when rate limited", func(t *testing.T) {
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{Rate: 10, Burst: 1})
		defer rl.Stop()
...
func TestPerIPLimiter_GetClientIP(t *testing.T) {
	t.Run("without trusted proxies", func(t *testing.T) {
		// By default, proxy headers are NOT trusted (secure default)
		rl := ratelimit.NewPerIPLimiter(ratelimit.PerIPConfig{Rate: 10, Burst: 10})
		defer rl.Stop()
	...
	t.Run("with trust all proxies", func(t *testing.T) {
		...
		if got != "203.0.113.195" {
			t.Errorf("ClientIP() = %v, want 203.0.113.195", got)
		}
	})
}
