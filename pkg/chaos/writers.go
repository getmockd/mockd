...
// JitterWriter adds random jitter between chunks
...
// NewJitterWriter creates a writer that adds jitter between chunks
func NewJitterWriter(w http.ResponseWriter, minJitter, maxJitter time.Duration, chunkSize int, rng *rand.Rand) *JitterWriter {
	return &JitterWriter{
		w:         w,
		minJitter: minJitter,
		maxJitter: maxJitter,
...
// Header returns the header map
func (jw *JitterWriter) Header() http.Header {
	return jw.w.Header()
}

// WriteHeader writes the status code
func (jw *JitterWriter) WriteHeader(statusCode int) {
	jw.w.WriteHeader(statusCode)
}

// Write writes bytes in chunks with random jitter between them
func (jw *JitterWriter) Write(p []byte) (int, error) {
	totalWritten := 0

	for len(p) > 0 {
		chunkSize := jw.chunkSize
		if chunkSize > len(p) {
			chunkSize = len(p)
		}

		n, err := jw.w.Write(p[:chunkSize])
		totalWritten += n
		if err != nil {
			return totalWritten, err
		}
		...
		if len(p) > 0 {
			jw.mu.Lock()
			jitter := jw.minJitter
			if jw.maxJitter > jw.minJitter {
				jitter += time.Duration(jw.rng.Int63n(int64(jw.maxJitter - jw.minJitter)))
			}
			jw.mu.Unlock()
			time.Sleep(jitter)
		}
...
// Unwrap returns the underlying ResponseWriter
func (jw *JitterWriter) Unwrap() http.ResponseWriter {
	return jw.w
}
