...
func convertEndpointToMock(ep *NativeV1Endpoint, defaultDelay int, now time.Time) (*config.MockConfiguration, error) {
	...
	// Set match priority
	if ep.Match != nil {
		mock.Priority = ep.Match.Priority
	}

	// Convert request matching criteria
	if ep.Request != nil {
		mock.Matcher.Headers = ep.Request.Headers
		mock.Matcher.QueryParams = ep.Request.Query
		mock.Matcher.BodyContains = ep.Request.BodyContains
		mock.Matcher.BodyEquals = ep.Request.BodyEquals
		mock.Matcher.BodyPattern = ep.Request.BodyPattern
	}
	...
	// Convert SSE config
	if ep.SSE != nil {
		mock.SSE = &config.SSEConfig{
			Template: ep.SSE.Template,
		}
		for _, event := range ep.SSE.Events {
			mock.SSE.Events = append(mock.SSE.Events, config.SSEEventDef{
				Type:    event.Type,
				Data:    event.Data,
				ID:      event.ID,
				Retry:   event.Retry,
	...
	if ep.Chunked != nil {
		mock.Chunked = &config.ChunkedConfig{
...
func MockCollectionToNativeV1(collection *config.MockCollection) (*NativeV1, error) {
	...
	if collection.ServerConfig != nil {
		native.Settings.Server = &NativeV1Server{
			...
			AutoGenerateCert: collection.ServerConfig.AutoGenerateCert,
		}
...
func convertMockToEndpoint(mock *config.MockConfiguration) (*NativeV1Endpoint, error) {
	ep := &NativeV1Endpoint{
		ID:      mock.ID,
		Name:    mock.Name,
	...
	// Convert matcher
	if mock.Matcher != nil {
		ep.Method = mock.Matcher.Method
		ep.Path = mock.Matcher.Path

		// Only include request if there are additional matching criteria
		if len(mock.Matcher.Headers) > 0 || len(mock.Matcher.QueryParams) > 0 ||
			mock.Matcher.BodyContains != "" || mock.Matcher.BodyEquals != "" ||
			mock.Matcher.BodyPattern != "" {
			ep.Request = &NativeV1Request{
				Headers:      mock.Matcher.Headers,
				Query:        mock.Matcher.QueryParams,
				BodyContains: mock.Matcher.BodyContains,
				BodyEquals:   mock.Matcher.BodyEquals,
				BodyPattern:  mock.Matcher.BodyPattern,
			}
		}
	}
	...
	if mock.Response != nil {
		...
		if mock.Response.Body != "" {
			...
			if err := json.Unmarshal([]byte(mock.Response.Body), &parsed); err == nil {
				ep.Response.Body = parsed
			} else {
				ep.Response.Body = mock.Response.Body
			}
		}
	}
	...
	if mock.SSE != nil {
		ep.SSE = &NativeV1SSE{
			Template: mock.SSE.Template,
		}
		for _, event := range mock.SSE.Events {
		...
		if mock.SSE.Timing.FixedDelay != nil || mock.SSE.Timing.InitialDelay != 0 {
			ep.SSE.Timing = &NativeV1SSETiming{
				FixedDelay:   mock.SSE.Timing.FixedDelay,
				InitialDelay: mock.SSE.Timing.InitialDelay,
			}
		}
		if mock.SSE.Lifecycle.KeepaliveInterval != 0 || mock.SSE.Lifecycle.MaxEvents != 0 || mock.SSE.Lifecycle.Timeout != 0 {
			ep.SSE.Lifecycle = &NativeV1Lifecycle{
				KeepaliveInterval: mock.SSE.Lifecycle.KeepaliveInterval,
				MaxEvents:         mock.SSE.Lifecycle.MaxEvents,
				Timeout:           mock.SSE.Lifecycle.Timeout,
			}
		}
	}
	...
	if mock.Chunked != nil {
		ep.Chunked = &NativeV1Chunked{
			ChunkSize:  mock.Chunked.ChunkSize,
			ChunkDelay: mock.Chunked.ChunkDelay,
			Data:       mock.Chunked.Data,
			Format:     mock.Chunked.Format,
		}
	}
