...
func (i *OpenAPIImporter) operationToMock(path, method string, op *Operation, id int, now time.Time) *config.MockConfiguration {
	...
	if mock.Name == "" {
		mock.Name = fmt.Sprintf("%s %s", method, path)
	}
	...
	if len(queryParams) > 0 {
		mock.Matcher.QueryParams = queryParams
	}
	...
	if response != nil {
		for contentType, mediaType := range response.Content {
			mock.Response.Headers["Content-Type"] = contentType
			if mediaType.Example != nil {
				bodyBytes, _ := json.MarshalIndent(mediaType.Example, "", "  ")
				mock.Response.Body = string(bodyBytes)
	...
	if mock.Response.Body == "" {
		mock.Response.Body = generateDefaultBody(statusCode)
	}
...
func (i *OpenAPIImporter) swaggerOperationToMock(path, method string, op *SwaggerOperation, id int, now time.Time) *config.MockConfiguration {
	...
	if len(op.Produces) > 0 {
		mock.Response.Headers["Content-Type"] = op.Produces[0]
	}
...
func (e *OpenAPIExporter) Export(collection *config.MockCollection) ([]byte, error) {
	...
	for _, mock := range collection.Mocks {
		...
		path := convertMockdPath(mock.Matcher.Path)
		pathMocks[path] = append(pathMocks[path], mock)
	}
	...
	for path, mocks := range pathMocks {
		...
		for _, mock := range mocks {
			op := e.mockToOperation(mock)
			switch strings.ToUpper(mock.Matcher.Method) {
			case "GET":
				pathItem.Get = op
			case "POST":
				pathItem.Post = op
...
func (e *OpenAPIExporter) mockToOperation(mock *config.MockConfiguration) *Operation {
	op := &Operation{
		Summary:   mock.Name,
	...
	if mock.Matcher != nil && len(mock.Matcher.QueryParams) > 0 {
		for name, value := range mock.Matcher.QueryParams {
	...
	if mock.Response != nil {
		statusStr := fmt.Sprintf("%d", mock.Response.StatusCode)
		response := Response{
			Description: fmt.Sprintf("%d response", mock.Response.StatusCode),
		}
		...
		if mock.Response.Body != "" {
			...
			if err := json.Unmarshal([]byte(mock.Response.Body), &example); err == nil {
				response.Content = map[string]MediaType{
					contentType: {Example: example},
				}
			} else {
				response.Content = map[string]MediaType{
					contentType: {Example: mock.Response.Body},
				}
			}
		}
