...
func generateCRUDTemplate(params map[string]string) (*config.MockCollection, error) {
	...
	sampleItem := fmt.Sprintf(`{"id": 1, "name": "Sample %s"}`, strings.Title(strings.TrimSuffix(resource, "s")))
	sampleList := fmt.Sprintf(`[%s, {"id": 2, "name": "Another %s"}]`, sampleItem, strings.Title(strings.TrimSuffix(resource, "s")))
	...
	collection.Mocks[2].Response.Headers["Location"] = basePath + "/1"

	// No content for delete
	collection.Mocks[4].Response.Headers = map[string]string{}
