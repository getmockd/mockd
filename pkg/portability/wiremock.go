...
func (i *WireMockImporter) mappingToMock(mapping WireMockMapping, id int, now time.Time) *config.MockConfiguration {
	...
	if mock.Name == "" {
		mock.Name = fmt.Sprintf("WireMock Import %d", id)
	}
	...
	// Method
	mock.Matcher.Method = req.Method
	if mock.Matcher.Method == "" {
		mock.Matcher.Method = "GET" // Default to GET
	}

	// URL path - prefer urlPath over url over urlPattern
	if req.URLPath != "" {
		mock.Matcher.Path = req.URLPath
	} else if req.URL != "" {
		mock.Matcher.Path = req.URL
	} else if req.URLPattern != "" {
		mock.Matcher.PathPattern = req.URLPattern
	} else if req.URLPathPattern != "" {
		mock.Matcher.PathPattern = req.URLPathPattern
	}
	...
	if len(req.BodyPatterns) > 0 {
		...
		if bp.EqualTo != "" {
			mock.Matcher.BodyEquals = bp.EqualTo
		} else if bp.Contains != "" {
			mock.Matcher.BodyContains = bp.Contains
		} else if bp.Matches != "" {
			mock.Matcher.BodyPattern = bp.Matches
		}
	}
	...
	for name, value := range resp.Headers {
		mock.Response.Headers[name] = value
	}

	// Response body
	if resp.Body != "" {
		mock.Response.Body = resp.Body
	} else if resp.JSONBody != nil {
		bodyBytes, _ := json.MarshalIndent(resp.JSONBody, "", "  ")
		mock.Response.Body = string(bodyBytes)
		if _, ok := mock.Response.Headers["Content-Type"]; !ok {
