...
// messageResponseFromConfig converts config.WSMessageResponse to MessageResponse.
func messageResponseFromConfig(cfg *config.WSMessageResponse) (*MessageResponse, error) {
	if cfg == nil {
		return nil, nil
	}
...
// scenarioFromConfig converts config.WSScenarioConfig to ScenarioConfig.
func scenarioFromConfig(cfg *config.WSScenarioConfig) (*ScenarioConfig, error) {
	if cfg == nil {
		return nil, nil
	}
	...
	for _, step := range cfg.Steps {
		stepCfg, err := scenarioStepFromConfig(step)
		if err != nil {
			return nil, err
		}
...
// scenarioStepFromConfig converts config.WSScenarioStepConfig to ScenarioStepConfig.
func scenarioStepFromConfig(cfg *config.WSScenarioStepConfig) (*ScenarioStepConfig, error) {
	if cfg == nil {
		return nil, nil
	}
...
// heartbeatFromConfig converts config.WSHeartbeatConfig to HeartbeatConfig.
func heartbeatFromConfig(cfg *config.WSHeartbeatConfig) (*HeartbeatConfig, error) {
	if cfg == nil {
		return nil, nil
	}
