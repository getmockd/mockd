...
func TestRecordingsConvertToMocks(t *testing.T) {
	...
	if mock.Matcher.Method != "POST" {
		t.Errorf("Expected method POST, got %s", mock.Matcher.Method)
	}

	if mock.Matcher.Path != "/users" {
		t.Errorf("Expected path /users, got %s", mock.Matcher.Path)
	}

	if mock.Response.StatusCode != 201 {
		t.Errorf("Expected status 201, got %d", mock.Response.StatusCode)
	}

	if mock.Response.Body != `{"id": 1, "name": "John"}` {
		t.Errorf("Unexpected body: %s", mock.Response.Body)
	}
	...
	if mockWithHeaders.Matcher.Headers == nil {
		t.Error("Expected headers to be included")
	}
	if mockWithHeaders.Matcher.Headers["Content-Type"] != "application/json" {
		t.Errorf("Expected Content-Type header, got %v", mockWithHeaders.Matcher.Headers)
	}
}
...
func TestConvertSession(t *testing.T) {
	...
	for _, mock := range mocks {
		...
		if mock.Matcher == nil {
			t.Error("Expected mock to have a matcher")
		}
		if mock.Response == nil {
			t.Error("Expected mock to have a response")
		}
	}
}
