...
// testServer creates a test MCP server with a mock engine.
func testServer(t *testing.T) (*mcp.Server, *engine.Server) {
	t.Helper()
	...
	mcpServer := mcp.NewServer(mcpCfg, eng)

	return mcpServer, eng
}
...
func TestMCP_GetMockData(t *testing.T) {
	mcpServer, eng := testServer(t)
	handler := mcpServer.Handler()

	// Add a mock to the engine
	...
	if err := eng.AddMock(mock); err != nil {
