...
func createStatefulServerWithAdmin(t *testing.T, resources ...*statefulResourceConfig) *testServerBundle {
	...
	// Register each stateful resource using the server's API
	for _, res := range resources {
		err := srv.RegisterStatefulResource(&config.StatefulResourceConfig{
			Name:        res.Name,
		...
		require.NoError(t, err, "failed to register resource %s", res.Name)
	}
...
func TestStateful_US4_GetStateReturnsOverview(t *testing.T) {
	...
	var overview stateful.StateOverview
	err = json.NewDecoder(resp2.Body).Decode(&overview)
	require.NoError(t, err)

	assert.Equal(t, 2, overview.Resources, "should have 2 resources")
...
func TestStateful_US4_GetResourceDetailsReturnsInfo(t *testing.T) {
	...
	var info stateful.ResourceInfo
