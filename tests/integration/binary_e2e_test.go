...
func TestBinaryE2E_MQTTRetainedMessages(t *testing.T) {
	...
	t.Run("RetainedMessageDelivery", func(t *testing.T) {
		...
		time.Sleep(200 * time.Millisecond) // Allow retained message to be stored
	...
	t.Run("RetainedMessageOverwrite", func(t *testing.T) {
		...
		time.Sleep(100 * time.Millisecond) // Allow retained message to be stored
	...
	t.Run("RetainedMessageClear", func(t *testing.T) {
		...
		time.Sleep(200 * time.Millisecond) // Allow retained message clear to propagate
	...
	t.Run("RetainedMessagesWithWildcard", func(t *testing.T) {
		...
		time.Sleep(300 * time.Millisecond) // Allow retained messages to be stored
...
func TestBinaryE2E_HTTPSAutoGeneratedCert(t *testing.T) {
	...
	for time.Now().Before(deadline) {
		...
		lastErr = err
		time.Sleep(200 * time.Millisecond) // Allow HTTPS server to start
	}
