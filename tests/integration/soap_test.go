...
func TestSOAP_US3_WSDLAutoGenerated(t *testing.T) {
	cfg := &soap.SOAPConfig{
		ID:      "test-no-wsdl",
	...
	// Request WSDL — even without explicit WSDL config, a minimal WSDL
	// is auto-generated from the configured operations
	resp, err := http.Get(bundle.BaseURL + "?wsdl")
	require.NoError(t, err)
	body, _ := io.ReadAll(resp.Body)
	resp.Body.Close()

	// Auto-generated WSDL should return 200 and contain the operation name
	assert.Equal(t, http.StatusOK, resp.StatusCode)
	assert.Contains(t, string(body), "GetUser")
}
