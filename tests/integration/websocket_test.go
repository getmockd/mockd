...
func setupTestServer(t *testing.T, wsEndpoints []*config.WebSocketEndpointConfig) *httptest.Server {
	...
	for _, ep := range wsEndpoints {
		err := srv.RegisterWebSocketEndpoint(ep)
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			Matchers: []*config.WSMatcherConfig{
				{
					Match:    &config.WSMatchCriteria{Type: "regex", Value: "^hello.*"},
					Response: &config.WSMessageResponse{Type: "text", Value: "hi there!"},
				},
			},
		},
	})
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			Matchers: []*config.WSMatcherConfig{
				{
					Match:    &config.WSMatchCriteria{Type: "json", Path: "$.type", Value: "subscribe"},
					Response: &config.WSMessageResponse{Type: "json", Value: map[string]interface{}{"status": "subscribed"}},
				},
			},
		},
	})
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			DefaultResponse: &config.WSMessageResponse{Type: "text", Value: "unknown command"},
		},
	})
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			Matchers: []*config.WSMatcherConfig{
				{
					Match:    &config.WSMatchCriteria{Type: "exact", Value: "test"},
					Response: &config.WSMessageResponse{Type: "text", Value: "first"},
				},
				{
					Match:    &config.WSMatchCriteria{Type: "contains", Value: "test"},
					Response: &config.WSMessageResponse{Type: "text", Value: "second"},
				},
			},
		},
	})
...
func TestWS_US3_ScenarioSendsOnConnect(t *testing.T) {
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			Path: "/ws/welcome",
			Scenario: &config.WSScenarioConfig{
				Name: "welcome",
				Steps: []*config.WSScenarioStepConfig{
					{Type: "send", Message: &config.WSMessageResponse{Type: "text", Value: "Welcome!"}},
				},
			},
		},
	})
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			Scenario: &config.WSScenarioConfig{
				Name: "delayed",
				Steps: []*config.WSScenarioStepConfig{
					{Type: "send", Message: &config.WSMessageResponse{Type: "text", Value: "first"}},
					{Type: "wait", Duration: "100ms"},
					{Type: "send", Message: &config.WSMessageResponse{Type: "text", Value: "second"}},
				},
			},
		},
	})
...
func TestWS_US6_ServerPing(t *testing.T) {
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			Path: "/ws/heartbeat",
			Heartbeat: &config.WSHeartbeatConfig{
				Enabled:  true,
				Interval: "100ms",
				Timeout:  "50ms",
			},
	...
	ts := setupTestServer(t, []*config.WebSocketEndpointConfig{
		{
			...
			Matchers: []*config.WSMatcherConfig{
				{
					Match:    &config.WSMatchCriteria{Type: "contains", Value: "needle"},
					Response: &config.WSMessageResponse{Type: "text", Value: "found"},
				},
			},
		},
	})
...
func TestWS_AdminAPI_Broadcast(t *testing.T) {
	...
	sent := manager.Broadcast("/ws/broadcast", websocket.MessageText, []byte("broadcast!"))
...
func TestWS_AdminAPI_Groups(t *testing.T) {
	...
	// Add conn1 to a group
	err := manager.JoinGroup(conn1ID, "room:test")
	require.NoError(t, err)

	// Broadcast to group - only conn1 should receive
	sent := manager.BroadcastToGroup("room:test", websocket.MessageText, []byte("group msg"))
	assert.Equal(t, 1, sent)

	msg := readText(t, conn1)
	assert.Equal(t, "group msg", msg)

	// Add conn2 to group
	err = manager.JoinGroup(conn2ID, "room:test")
	require.NoError(t, err)

	// Now both should receive
	sent = manager.BroadcastToGroup("room:test", websocket.MessageText, []byte("group msg 2"))
	assert.Equal(t, 2, sent)

	assert.Equal(t, "group msg 2", readText(t, conn1))
	assert.Equal(t, "group msg 2", readText(t, conn2))
}
