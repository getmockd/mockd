...
# Delete a user
curl -X DELETE http://localhost:8080/api/users/user-003
```
...
# Pagination
curl "http://localhost:8080/api/products?limit=2&offset=0"
```
...
curl http://localhost:8080/api/users/user-001/orders

# Create an order for a user (userId is auto-set from path)
curl -X POST http://localhost:8080/api/users/user-001/orders \
  -H "Content-Type: application/json" \
  -d '{"productId": "prod-001", "quantity": 3, "status": "pending"}'

# Get a specific order
curl http://localhost:8080/api/users/user-001/orders/order-001
```
...
curl http://localhost:9090/state/resources

# Get details for a specific resource
curl http://localhost:9090/state/resources/users
```
...
curl -X POST http://localhost:9090/state/reset

# Reset only a specific resource
curl -X POST "http://localhost:9090/state/reset?resource=users"

# Clear a resource (removes all items, no seed data restore)
curl -X DELETE http://localhost:9090/state/resources/products
```
