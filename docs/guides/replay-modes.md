...
```bash
# Via Admin API
curl -X POST http://localhost:9090/stream-recordings/01HXYZ123456/replay \
...
```bash
# Pause
curl -X POST http://localhost:9090/replay/SESSION_ID/pause

# Resume
curl -X POST http://localhost:9090/replay/SESSION_ID/resume
```
...
curl -X POST http://localhost:9090/replay/SESSION_ID/advance
...
```bash
curl http://localhost:9090/replay/SESSION_ID
```
...
```bash
curl http://localhost:9090/replay
```

### Stop Replay

```bash
curl -X DELETE http://localhost:9090/replay/SESSION_ID
```
...
```bash
# 10x speed
curl -X POST http://localhost:9090/stream-recordings/REC_ID/replay \
...
```bash
# Start triggered replay
REPLAY_ID=$(curl -s -X POST http://localhost:9090/stream-recordings/REC_ID/replay \
  -d '{"mode": "triggered"}' | jq -r '.id')

# Step through
while true; do
  result=$(curl -s -X POST "http://localhost:9090/replay/$REPLAY_ID/advance")
  echo "$result" | jq
...
- [WebSocket Mocking](../reference/websocket.md) - WebSocket mock configuration
