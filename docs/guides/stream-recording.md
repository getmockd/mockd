...
```bash
curl -X POST http://localhost:9090/stream-recordings/start \
...
```bash
curl -X POST http://localhost:9090/stream-recordings/01HXYZ123456/stop
```
...
```bash
# All recordings
curl http://localhost:9090/stream-recordings

# Filter by protocol
curl "http://localhost:9090/stream-recordings?protocol=websocket"

# Filter by path
curl "http://localhost:9090/stream-recordings?path=/ws/chat"
```

### Get Recording Details

```bash
curl http://localhost:9090/stream-recordings/01HXYZ123456
```

### Export Recording

```bash
curl -X POST http://localhost:9090/stream-recordings/01HXYZ123456/export \
  -o recording.json
```
...
```bash
curl -X POST http://localhost:9090/stream-recordings/01HXYZ123456/convert \
  -H "Content-Type: application/json" \
...
WS_URL=ws://localhost:8080 npm test
