...
# By assignee
curl "http://localhost:8080/api/tasks?assigneeId=1"
```
...
```bash
curl -X PATCH http://localhost:8080/api/tasks/4 \
  -H "Content-Type: application/json" \
  -d '{"status": "done"}'
```
...
```bash
curl http://localhost:8081/__admin/state
```
...
curl -X DELETE http://localhost:8081/__admin/state/tasks
```
...
describe('Tasks API', () => {
  beforeEach(async () => {
    ...
    await fetch('http://localhost:8081/__admin/state', {
      method: 'DELETE'
    });
  });
...
ADMIN = 'http://localhost:8081/__admin'
