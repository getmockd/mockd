...
beforeEach(async () => {
  // Reset state
  await fetch('http://localhost:8081/__admin/state', { method: 'DELETE' });
});
...
describe('Error Handling', () => {
  test('handles server errors gracefully', async () => {
    ...
    await fetch('http://localhost:8081/__admin/mocks', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
def reset_state():
    ...
    requests.delete("http://localhost:8081/__admin/state")
```
...
var adminURL = "http://localhost:8081/__admin"
...
- [Basic Mocks](basic-mocks.md) - Simple mock examples
- [CRUD API](crud-api.md) - Stateful API example
